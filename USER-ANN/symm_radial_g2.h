...
struct PhiR_G2: public PhiR{
	...
	inline double val(double r)const{return std::exp(-eta*(r-rs)*(r-rs))*CutoffF::funcs[tcut](r,rc);};
	inline double grad(double r)const{return std::exp(-eta*(r-rs)*(r-rs))*(-2.0*eta*(r-rs)*CutoffF::funcs[tcut](r,rc)+CutoffFD::funcs[tcut](r,rc));};
};
