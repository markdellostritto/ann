...
struct PhiA_G4: public PhiA{
	...
	double operator()(double cos, double ri, double rj, double rij)const noexcept final;
	//inline double val(double cos, double rij, double rik, double rjk)const noexcept final{return angle(cos)*dist(rij,rik,rjk);};
	inline double val(double cos, double rij, double rik, double rjk)const noexcept final{
	...
	inline double angle(double cos)const noexcept final{return (std::fabs(cos+1)<num_const::ZERO)?0:2.0*std::pow(0.5*(1.0+lambda*cos),zeta);};
	inline double dist(double rij, double rik, double rjk)const noexcept final{return std::exp(-eta*(rij*rij+rik*rik))*CutoffF::funcs[tcut](rij,rc)*CutoffF::funcs[tcut](rik,rc);};
	inline double grad_angle(double cos)const noexcept final{return (std::fabs(cos+1)<num_const::ZERO)?0:zeta*lambda*std::pow(0.5*(1.0+lambda*cos),zeta-1.0);};
	inline double grad_dist_0(double rij, double rik, double rjk)const noexcept final
	...
	inline double grad_dist_1(double rij, double rik, double rjk)const noexcept final
	...
	inline double grad_dist_2(double rij, double rik, double rjk)const noexcept final{return 0.0;};
};
