...
void Coulomb::init(const Structure& struc, double prec){
	...
	int rNMaxX=std::ceil(rMax_/struc.R().row(0).lpNorm<Eigen::Infinity>());//max of row - max abs x the lattice vectors
	int rNMaxY=std::ceil(rMax_/struc.R().row(1).lpNorm<Eigen::Infinity>());//max of row - max abs y the lattice vectors
	int rNMaxZ=std::ceil(rMax_/struc.R().row(2).lpNorm<Eigen::Infinity>());//max of row - max abs z the lattice vectors
	int kNMaxX=std::ceil(kMax_/struc.K().row(0).lpNorm<Eigen::Infinity>());//max of row - max abs x the lattice vectors
	int kNMaxY=std::ceil(kMax_/struc.K().row(1).lpNorm<Eigen::Infinity>());//max of row - max abs y the lattice vectors
	int kNMaxZ=std::ceil(kMax_/struc.K().row(2).lpNorm<Eigen::Infinity>());//max of row - max abs z the lattice vectors
	...
	for(int i=kCount-1; i>=0; --i){
		#if (defined(__GNUC__) || defined(__GNUG__)) && !(defined(__clang__) || defined(__INTEL_COMPILER))
		kAmp[i]=4.0*num_const::PI/struc.vol()*std::exp(-K[i].dot(K[i])/(4.0*alpha_*alpha_))/(K[i].dot(K[i]));
		#elif defined __ICC || defined __INTEL_COMPILER
		kAmp[i]=4.0*num_const::PI/struc.vol()*exp(-K[i].dot(K[i])/(4.0*alpha_*alpha_))/(K[i].dot(K[i]));
		#endif
	}
...
void Coulomb::init_alpha(const Structure& struc, double prec){
	...
	alpha_=std::pow(struc.nAtoms()*weight_*num_const::PI*num_const::PI*num_const::PI/(struc.vol()*struc.vol()),1.0/6.0);
...
double Coulomb::energy(const Structure& struc)const{
	...
	if(EWALD_PRINT_DATA==0){
		...
		//polarization energy
		energyT+=2.0*num_const::PI/(2.0*eps_+1.0)*vec.dot(vec)/struc.vol();
	} else {
		...
		//self-energy 
		for(int i=struc.nAtoms()-1; i>=0; --i) q2s+=struc.charge(i)*struc.charge(i);
		energyS=0.5*q2s*(vSelfR_-vSelfC_);
		//polarization energy
		vec.setZero();
		for(int i=struc.nAtoms()-1; i>=0; --i) vec.noalias()+=struc.charge(i)*struc.posn(i);
		energyP=2.0*num_const::PI/(2.0*eps_+1.0)*vec.dot(vec)/struc.vol();
...
double Coulomb::energy_single(const Structure& struc){
	...
	for(int i=-shellx; i<=shellx; ++i){
		for(int j=-shelly; j<=shelly; ++j){
			for(int k=-shellz; k<=shellz; ++k){
				Eigen::Vector3d vtemp=i*struc.K().col(0)+j*struc.K().col(1)+k*struc.K().col(2);
	...
	for(int n=K.size()-1; n>=0; --n){
		...
		const double kamp=4.0*num_const::PI/struc.vol()*std::exp(-kdot/(4.0*alpha_*alpha_))/kdot;
		for(int i=struc.nAtoms()-1; i>=0; --i){
			#if (defined(__GNUC__) || defined(__GNUG__)) && !(defined(__clang__) || defined(__INTEL_COMPILER))
			sf+=struc.charge(i)*std::exp(-I*K[n].dot(struc.posn(i)));
			#elif defined __ICC || defined __INTEL_COMPILER
	...
	energyS=0.5*q2s*(vSelfR-2.0*alpha_/num_const::RadPI);
...
double Coulomb::energy_brute(const Structure& struc, int N)const{
	...
	for(int i=-N; i<=N; ++i){
		for(int j=-N; j<=N; ++j){
			for(int k=-N; k<=N; ++k){
				double norm=(i*struc.R().col(0)+j*struc.R().col(1)+k*struc.R().col(2)).norm();
...
double Coulomb::phi(const Structure& struc, const Eigen::Vector3d& r)const{
	...
	if(EWALD_PRINT_DATA==0){
		...
		vTot+=-2.0*num_const::PI/(2.0*eps_+1.0)*r2/struc.vol();
		//constant contribution
		vTot+=-num_const::PI/(alpha_*alpha_*struc.vol())*chgsum;
	} else {
		...
		//polarization contribution
		vP=-2.0*num_const::PI/(2.0*eps_+1.0)*r2/struc.vol();
		//constant contribution
		vC=-num_const::PI/(alpha_*alpha_*struc.vol())*chgsum;
...
Eigen::Vector3d& Coulomb::efieldBrute(const Structure& struc, unsigned int n, Eigen::Vector3d& field, int N)const{
	if(EWALD_PRINT_FUNC>0) std::cout<<"potentialBrute(const Structure&,int,Eigen::Vector3d&,unsigned int)const:\n";
	Eigen::Vector3d dr;
	...
	for(unsigned int ii=0; ii<struc.nAtoms(); ++ii){
		...
		for(int i=-N; i<=N; ++i){
			for(int j=-N; j<=N; ++j){
				for(int k=-N; k<=N; ++k){
					double drn=(dr+i*struc.R().col(0)+j*struc.R().col(1)+k*struc.R().col(2)).norm();
