...
*/
BasisR::BasisR(const BasisR& basis):fR_(NULL),cutoff_(NULL){
...
std::ostream& operator<<(std::ostream& out, const BasisR& basis){
	if(basis.cutoff()!=NULL) out<<"BasisR "<<basis.cutoff_->name()<<" "<<basis.rc_<<" "<<basis.phiRN_<<" "<<basis.nfR_;
...
*/
void BasisR::init_G1(int nR, cutoff::Name::type tcut, double rcut){
...
* @param rcut - the cutoff distance
*/
void BasisR::init_G2(int nR, cutoff::Name::type tcut, double rcut){
...
* @param rcut - the cutoff distance
*/
void BasisR::init_T1(int nR, cutoff::Name::type tcut, double rcut){
...
void BasisR::write(FILE* writer,const BasisR& basis){
	...
	fprintf(writer,"BasisR %s %f %s %i\n",str_tcut,basis.rc(),str_phirn,basis.nfR());
...
void BasisR::read(FILE* reader, BasisR& basis){
	...
	if(strlist.size()!=5) throw std::runtime_error("BasisR::read(FILE*,BasisR&): Invalid BasisR format.");
	//read header
	const cutoff::Name::type tcut=cutoff::Name::read(strlist[1].c_str());
	const double rc=std::atof(strlist[2].c_str());
	const int nfr=std::atoi(strlist[4].c_str());
...
*/
double BasisR::norm(double rc){
	return 1.0/(0.5*4.0/3.0*(PI*PI-6.0)/PI*rc*rc*rc);
}
