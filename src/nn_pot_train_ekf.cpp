...
template <> int nbytes(const NNPotOpt& obj){
	...
	//optimization
		...
		size+=sizeof(int);//decayg_
		size+=sizeof(int);//decayn_
		size+=nbytes(obj.ekf_);
	//nn
...
template <> int pack(const NNPotOpt& obj, char* arr){
	...
	//optimization
		std::memcpy(arr+pos,&obj.max_,sizeof(int)); pos+=sizeof(int);
		std::memcpy(arr+pos,&obj.nPrint_,sizeof(int)); pos+=sizeof(int);
		std::memcpy(arr+pos,&obj.nWrite_,sizeof(int)); pos+=sizeof(int);
		std::memcpy(arr+pos,&obj.step_,sizeof(int)); pos+=sizeof(int);
		std::memcpy(arr+pos,&obj.count_,sizeof(int)); pos+=sizeof(int);
		std::memcpy(arr+pos,&obj.period_,sizeof(int)); pos+=sizeof(int);
		std::memcpy(arr+pos,&obj.tol_,sizeof(double)); pos+=sizeof(double);
		std::memcpy(arr+pos,&obj.gamma_,sizeof(double)); pos+=sizeof(double);
		std::memcpy(arr+pos,&obj.alpha_,sizeof(double)); pos+=sizeof(double);
		std::memcpy(arr+pos,&obj.power_,sizeof(double)); pos+=sizeof(double);
		std::memcpy(arr+pos,&obj.decayg_,sizeof(int)); pos+=sizeof(int);
		std::memcpy(arr+pos,&obj.decayn_,sizeof(int)); pos+=sizeof(int);
		pos+=pack(obj.ekf_,arr+pos);
	//nn
...
template <> int unpack(NNPotOpt& obj, const char* arr){
	...
	//optimization
		std::memcpy(&obj.max_,arr+pos,sizeof(int)); pos+=sizeof(int);
		std::memcpy(&obj.nPrint_,arr+pos,sizeof(int)); pos+=sizeof(int);
		std::memcpy(&obj.nWrite_,arr+pos,sizeof(int)); pos+=sizeof(int);
		std::memcpy(&obj.step_,arr+pos,sizeof(int)); pos+=sizeof(int);
		std::memcpy(&obj.count_,arr+pos,sizeof(int)); pos+=sizeof(int);
		std::memcpy(&obj.period_,arr+pos,sizeof(int)); pos+=sizeof(int);
		std::memcpy(&obj.tol_,arr+pos,sizeof(double)); pos+=sizeof(double);
		std::memcpy(&obj.gamma_,arr+pos,sizeof(double)); pos+=sizeof(double);
		std::memcpy(&obj.alpha_,arr+pos,sizeof(double)); pos+=sizeof(double);
		std::memcpy(&obj.power_,arr+pos,sizeof(double)); pos+=sizeof(double);
		std::memcpy(&obj.decayg_,arr+pos,sizeof(int)); pos+=sizeof(int);
		std::memcpy(&obj.decayn_,arr+pos,sizeof(int)); pos+=sizeof(int);
		pos+=unpack(obj.ekf_,arr+pos);
	//nn
...
std::ostream& operator<<(std::ostream& out, const NNPotOpt& nnPotOpt){
	...
	out<<"MAX_ITER     = "<<nnPotOpt.max_<<"\n";
	out<<"N_PRINT      = "<<nnPotOpt.nPrint_<<"\n";
	out<<"N_WRITE      = "<<nnPotOpt.nWrite_<<"\n";
	out<<"STEP         = "<<nnPotOpt.step_<<"\n";
	out<<"COUNT        = "<<nnPotOpt.count_<<"\n";
	out<<"PERIOD       = "<<nnPotOpt.period_<<"\n";
	out<<"TOL          = "<<nnPotOpt.tol_<<"\n";
	out<<"GAMMA        = "<<nnPotOpt.gamma_<<"\n";
	out<<"ALPHA        = "<<nnPotOpt.alpha_<<"\n";
	out<<"POWER        = "<<nnPotOpt.power_<<"\n";
	out<<"DECAYG       = "<<nnPotOpt.decayg_<<"\n";
	out<<"DECAYN       = "<<nnPotOpt.decayn_<<"\n";
...
void NNPotOpt::defaults(){
	...
	//optimization
		...
		alpha_=0;
		power_=0;
		decayg_=Opt::DECAY::UNKNOWN;
		decayn_=Opt::DECAY::UNKNOWN;
		ekf_.clear();
...
void NNPotOpt::update_step(int step){
	switch(decayg_){
...
void NNPotOpt::update_noise(int step){
	switch(decayn_){
		case Opt::DECAY::CONST: break;
		case Opt::DECAY::EXP:  ekf_.noise()*=exp(-alpha_); break;
		case Opt::DECAY::SQRT: ekf_.noise()*=sqrt((1.0+alpha_*step)/(1.0+alpha_*(step+1))); break;
		case Opt::DECAY::INV:  ekf_.noise()*=(1.0+alpha_*step)/(1.0+alpha_*(step+1)); break;
		case Opt::DECAY::POW:  ekf_.noise()*=pow((1.0+alpha_*step)/(1.0+alpha_*(step+1)),power_); break;
		case Opt::DECAY::STEP: if(step>0 && step%period_==0) ekf_.noise()*=alpha_;
		break;
		default: break;
	}
}

void NNPotOpt::train(parallel::Dist dist_batch){
	...
	//batch
	batch_.resize(dist_batch_.size(),0);
	...
	hElement_.resize(nElements_);
	hTotal_.resize(nElements_);
	hLocal_.resize(nElements_);
	eLocal_.resize(dist_batch_.nobj());
	eGlobal_.resize(dist_batch_.nobj());
	nParams_=0;
	for(int n=0; n<nElements_; ++n){
		const int nn_size=nnpot_.nnh(n).nn().size();
		pElement_[n]=Eigen::VectorXd::Zero(nn_size);
		hElement_[n]=Eigen::MatrixXd::Zero(nn_size,dist_batch_.nobj());
		hTotal_[n]=Eigen::MatrixXd::Zero(nn_size,dist_batch_.nobj());
		hLocal_[n]=Eigen::MatrixXd::Zero(nn_size,dist_batch_.nobj());
		nParams_+=nn_size;
	}
	hGlobal_.resize(nParams_,dist_batch_.nobj());
	//resize gradient objects
	if(NN_POT_TRAIN_PRINT_STATUS>0 && WORLD.rank()==0) std::cout<<"resizing gradient data\n";
	dOutDP_.resize(nElements_);
	for(int i=0; i<nElements_; ++i){
		dOutDP_[i].resize(nnpot_.nnh(i).nn());
	}
	...
	if(restart_){
		...
		if(nParams_!=ekf_.p().size()) throw std::runtime_error(
			std::string("NNPotOpt::train(int): Network has ")
			+std::to_string(nParams_)+std::string(" while ekf has ")
			+std::to_string(ekf_.p().size())+std::string(" parameters.")
		);
	} else {
		//from scratch
		if(WORLD.rank()==0) std::cout<<"starting from scratch\n";
		//resize the optimization objects
		ekf_.resize(nParams_,dist_batch.nobj());
	...
	//bcast parameters
	MPI_Bcast(ekf_.p().data(),ekf_.p().size(),MPI_DOUBLE,0,WORLD.label());
	...
	for(int iter=0; iter<max_; ++iter){
		...
		MPI_Reduce(eLocal_.data(),eGlobal_.data(),dist_batch_.nobj(),MPI_DOUBLE,MPI_SUM,0,WORLD.label());
		//accumulate gradient
		for(int n=0; n<nElements_; ++n){
			hTotal_[n].setZero();
			MPI_Reduce(hElement_[n].data(),hTotal_[n].data(),hElement_[n].size(),MPI_DOUBLE,MPI_SUM,0,WORLD.label());
		}
		if(WORLD.rank()==0){
			...
			//pack the gradient
			for(int i=0; i<dist_batch.nobj(); ++i){
				count=0;
				for(int n=0; n<nElements_; ++n){
					for(int m=0; m<nnpot_.nnh(n).nn().size(); ++m){
						hGlobal_(count++,i)=hTotal_[n](m,i);
					}
				}
			}
			//print/write error
			...
			if(step_%nWrite_==0){
				...
				//write restart file
				const std::string file_restart=file_restart_+"."+std::to_string(count_);
				this->write_restart(file_restart.c_str());
				//write potential file
				const std::string file_ann=file_ann_+"."+std::to_string(count_);
			...
			valOld_=val_;//set "old" value
			//check the break condition
			switch(optVal_){
				case Opt::VAL::FTOL_ABS: fbreak=(val_<tol_); break;
				case Opt::VAL::FTOL_REL: fbreak=(dv_<tol_); break;
			}
		}
		//bcast parameters
		MPI_Bcast(ekf_.p().data(),ekf_.p().size(),MPI_DOUBLE,0,WORLD.label());
	...
	if(NN_POT_TRAIN_PRINT_DATA>-1 && WORLD.rank()==0){
		...
		std::cout<<"OPT-VAL    = "<<val_<<"\n";
		std::cout<<"TIME-TRAIN = "<<time_train_avg<<"\n";
		if(NN_POT_TRAIN_PRINT_DATA>1){
			std::cout<<"p = "; for(int i=0; i<ekf_.p().size(); ++i) std::cout<<ekf_.p()[i]<<" "; std::cout<<"\n";
		}
		std::cout<<print::title("OPT - SUMMARY",str)<<"\n";
		std::cout<<print::buf(str)<<"\n";
		delete[] str;
...
double NNPotOpt::error(const Eigen::VectorXd& x){
	...
	for(int n=0; n<nElements_; ++n) hElement_[n].setZero();
	...
	for(int i=0; i<batch_.size(); ++i){
		for(int j=0; j<nElements_; ++j) hLocal_[j].setZero();
		...
		for(int n=0; n<dist_atom.size(); ++n){
			...
			dOutDP_[index].grad(nnpot_.nnh(index).nn());
			//add the gradient to the total
			const NeuralNet::ANN& nn_=nnpot_.nnh(index).nn();
			int c_=0;
			for(int l=0; l<nn_.nlayer(); ++l){
				for(int j=0; j<nn_.bias(l).size(); ++j){
					hLocal_[index](c_++,dist_batch_.index(i))+=1*dOutDP_[index].dodb()[0][l](j);
				}
			}
			for(int l=0; l<nn_.nlayer(); ++l){
				for(int j=0; j<nn_.edge(l).cols(); ++j){
					for(int k=0; k<nn_.edge(l).rows(); ++k){
						hLocal_[index](c_++,dist_batch_.index(i))+=1*dOutDP_[index].dodw()[0][l](k,j);
					}
				}
			}
		}
		...
		eLocal_[dist_batch_.index(i)]=(strucl.energy()-energyt)/strucl.nAtoms();
		//accumulate gradient across the BATCH communicator
		for(int j=0; j<nElements_; ++j){
			hTotal_[j].setZero();
			MPI_Allreduce(hLocal_[j].data(),hTotal_[j].data(),hTotal_[j].size(),MPI_DOUBLE,MPI_SUM,BATCH.label());
		}
		//compute the error and gradient
		const double dcda=(energyt-strucl.energy())/strucl.nAtoms();
		error_train_+=0.5*dcda*dcda;
		for(int j=0; j<nElements_; ++j){
			hElement_[j].noalias()+=hTotal_[j]/strucl.nAtoms();
		}
	}
	...
	error_val_*=batchsi;
	eLocal_*=batchsi;
	for(int j=0; j<nElements_; ++j) hElement_[j]*=batchsi;
...
int main(int argc, char* argv[]){
	...
	try{
		...
		if(WORLD.rank()==0){
			...
			while(fgets(input,string::M,reader)!=NULL){
				...
				if(strlist.at(0)=="PRE_COND"){//whether to precondition the inputs
					...
					nnPotOpt.max_=std::atoi(strlist.at(1).c_str());//max number of iterations
				} else if(strlist.at(0)=="MAX_ITER"){
					nnPotOpt.max_=std::atoi(strlist.at(1).c_str());//max number of iterations
					...
					nnPotOpt.step_=std::atoi(strlist.at(1).c_str());
				} else if(strlist.at(0)=="TOL"){
					nnPotOpt.tol_=std::atof(strlist.at(1).c_str());
				} else if(strlist.at(0)=="DECAYG"){
					nnPotOpt.decayg_=Opt::DECAY::read(string::to_upper(strlist.at(1)).c_str());
				} else if(strlist.at(0)=="DECAYN"){
					nnPotOpt.decayn_=Opt::DECAY::read(string::to_upper(strlist.at(1)).c_str());
				} else if(strlist.at(0)=="ALPHA"){
					nnPotOpt.alpha_=std::atof(strlist.at(1).c_str());
				} else if(strlist.at(0)=="GAMMA"){
					nnPotOpt.gamma_=std::atof(strlist.at(1).c_str());
				} else if(strlist.at(0)=="POWER"){
					nnPotOpt.power_=std::atof(strlist.at(1).c_str());
				} else if(strlist.at(0)=="NOISE"){
					nnPotOpt.ekf_.noise()=std::atof(strlist.at(1).c_str());
				} else if(strlist.at(0)=="EPS"){
					nnPotOpt.ekf_.eps()=std::atof(strlist.at(1).c_str());
				} 
				//writing
		...
		if(WORLD.rank()==0){
			std::cout<<nnPotOpt<<"\n";
			//std::cout<<nnPotOpt.ekf_<<"\n";
		}
		MPI_Barrier(WORLD.label());
		
		//======== bcast the optimization data ========
		if(WORLD.rank()==0) std::cout<<"bcasting optimization data\n";
		MPI_Bcast(&nnPotOpt.max_,1,MPI_INT,0,WORLD.label());
		MPI_Bcast(&nnPotOpt.nPrint_,1,MPI_INT,0,WORLD.label());
		MPI_Bcast(&nnPotOpt.nWrite_,1,MPI_INT,0,WORLD.label());
		MPI_Bcast(&nnPotOpt.step_,1,MPI_INT,0,WORLD.label());
		MPI_Bcast(&nnPotOpt.count_,1,MPI_INT,0,WORLD.label());
		MPI_Bcast(&nnPotOpt.period_,1,MPI_INT,0,WORLD.label());
		MPI_Bcast(&nnPotOpt.tol_,1,MPI_DOUBLE,0,WORLD.label());
		MPI_Bcast(&nnPotOpt.gamma_,1,MPI_DOUBLE,0,WORLD.label());
		MPI_Bcast(&nnPotOpt.alpha_,1,MPI_DOUBLE,0,WORLD.label());
		MPI_Bcast(&nnPotOpt.power_,1,MPI_DOUBLE,0,WORLD.label());
		MPI_Bcast(&nnPotOpt.decayg_,1,MPI_INT,0,WORLD.label());
		MPI_Bcast(&nnPotOpt.decayn_,1,MPI_INT,0,WORLD.label());
		parallel::bcast(WORLD.label(),0,nnPotOpt.ekf_);
