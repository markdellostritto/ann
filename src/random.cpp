...
double DistRayleigh::operator()(REngine& gen){
	return std::sqrt(-2.0*std::log(gen.randf()));
}
...
double DistLogistic::operator()(REngine& gen){
	const double p=gen.randf();
	return mu_+math::constant::Rad3/math::constant::PI*sigma_*std::log(p/(1.0-p));
}
...
double DistCauchy::operator()(REngine& gen){
	return mu_+sigma_*std::tan(math::constant::PI*(gen.randf()-0.5));
}
