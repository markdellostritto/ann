...
int main(int argc, char* argv[]){
	...
	//phonon - cell
		...
		double sqrtNinv=0;
		Structure pcell;//primitive unit cell
		Eigen::Vector3i nlat=Eigen::Vector3i::Zero();
		std::vector<Tensor<int> > atoms;//the atoms of the cell and their repeated positions (nprim_ x (n1*n2*n3))
	...
	try{
		...
		//======== read the pcell ========
		if(WORLD.rank()==0) std::cout<<"reading primitive cell\n";
		...
		//======== read the struc ========
		if(WORLD.rank()==0) std::cout<<"reading structure\n";
		...
		if(WORLD.rank()==0) std::cout<<"nAtoms = "<<nAtoms<<"\n";
		if(WORLD.rank()==0) std::cout<<"simulation occupying "<<nbytes/1000000.0<<" MB per proc\n";
		...
		//======== make the supercell ========
		if(WORLD.rank()==0) std::cout<<"making the super cell\n";
		Structure::super(pcell,struc,nlat);
