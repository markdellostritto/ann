...
void test_unit_rad(){
	...
	//radius
	const int nrad=10;
	const double rminNa=1.66/100.0;
	const double rminCl=1.02/100.0;
	const double rmaxNa=1.66;
	const double rmaxCl=1.02;
	const double drNa=(rmaxNa-rminNa)/(nrad-1.0);
	const double drCl=(rmaxCl-rminCl)/(nrad-1.0);
	std::vector<double> rvNa(nrad,0);
	std::vector<double> rvCl(nrad,0);
	for(int i=0; i<nrad; i++){
		rvNa[i]=rminNa+i*drNa;
		rvCl[i]=rminCl+i*drCl;
	}
	for(int i=0; i<nrad; ++i){
		const double rcut=6.0;
		const double alpha=0.5;
		//initialize the coul object
		ptnl::PotCoulDSF pot_coul;
		pot_coul.rc()=rcut;
		pot_coul.alpha()=alpha;
		//initialize the coul object
		ptnl::PotGaussDSF pot_gauss;
		pot_gauss.rc()=rcut;
		pot_gauss.alpha()=alpha;
		pot_gauss.resize(2);
		pot_gauss.radius(0)=rvNa[i];//Na
		pot_gauss.f(0)=1;
		pot_gauss.radius(1)=rvCl[i];//Cl
		pot_gauss.f(1)=1;
		pot_gauss.init();
		//compute the energy
		NeighborList nlist(struc,rcut);
		const double energyC=pot_coul.energy(struc,nlist);
		const double energyG=pot_gauss.energy(struc,nlist);
		std::cout<<"rad["<<i<<"] "<<rvNa[i]<<" "<<rvCl[i]<<" "<<energyC<<" "<<energyG<<"\n";
	}
}
...
int main(int argc, char* argv[]){
	...
	std::cout<<print::buf(str)<<"\n";
	std::cout<<print::title("POT_GAUSS_DSF",str)<<"\n";
	test_unit_rad();
	test_unit_J();
	//test_unit_pair();
	std::cout<<print::title("POT_GAUSS_DSF",str)<<"\n";
