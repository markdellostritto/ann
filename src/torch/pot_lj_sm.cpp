...
std::ostream& operator<<(std::ostream& out, const PotLJSm& pot){
	return out<<static_cast<const Pot&>(pot);
}
	
//==== member functions ====

void PotLJSm::read(Token& token){
	static_cast<Pot&>(*this).read(token);
}

void PotLJSm::coeff(Token& token){
	if(POT_LJ_SM_PRINT_FUNC>0) std::cout<<"ptnl::PotLJSm::coeff(Token&):\n";
...
void PotLJSm::resize(int ntypes){
	if(POT_LJ_SM_PRINT_FUNC>0) std::cout<<"ptnl::PotLJSm::resize(int):\n";
	if(ntypes<0) throw std::invalid_argument("ptnl::PotLJSm::resize(int): Invalid number of types.");
	ntypes_=ntypes;
	if(ntypes_>0){
		f_=Eigen::MatrixXi::Zero(ntypes_,ntypes_);
		s_=Eigen::MatrixXd::Zero(ntypes_,ntypes_);
		e_=Eigen::MatrixXd::Zero(ntypes_,ntypes_);
		phirc_=Eigen::MatrixXd::Zero(ntypes_,ntypes_);
		dphirc_=Eigen::MatrixXd::Zero(ntypes_,ntypes_);
	}
}

void PotLJSm::init(){
	if(POT_LJ_SM_PRINT_FUNC>0) std::cout<<"ptnl::PotLJSm::init():\n";
	...
	for(int i=0; i<ntypes_; ++i){
		for(int j=i; j<ntypes_; ++j){
			const double du=s_(i,j)/rc_;
			const double du3=du*du*du;
			const double du6=du3*du3;
			phirc_(i,j)=4.0*e_(i,j)*du6*(du6-1.0);
			phirc_(j,i)=phirc_(i,j);
			dphirc_(i,j)=-24.0*e_(i,j)*du6*(2.0*du6-1.0)/rc_;
			dphirc_(j,i)=dphirc_(i,j);
		}
	}
}

double PotLJSm::energy(const Structure& struc, const NeighborList& nlist){
	if(POT_LJ_SM_PRINT_FUNC>0) std::cout<<"ptnl::PotLJSm::energy(const Structure&,const NeighborList&):\n";
	double energy_=0;
	for(int i=0; i<struc.nAtoms(); ++i){
		...
		for(int j=0; j<nlist.size(i); ++j){
			...
			if(nlist.neigh(i,j).dr()<rc_){
				...
				const double du3=du*du*du;
				const double du6=du3*du3;
				energy_+=4.0*e_(ti,tj)*du6*(du6-1.0)-phirc_(ti,tj)-(dr-rc_)*dphirc_(ti,tj);
			}
		}
	}
	energy_*=0.5;
...
double PotLJSm::energy(const Structure& struc, const NeighborList& nlist, int i){
	if(POT_LJ_SM_PRINT_FUNC>0) std::cout<<"ptnl::PotLJSm::energy(const Structure&,const NeighborList&,int):\n";
	double energy_=0;
	const int ti=struc.type(i);
	for(int j=0; j<nlist.size(i); ++j){
		const int tj=struc.type(nlist.neigh(i,j).index());
...
double PotLJSm::compute(Structure& struc, const NeighborList& nlist){
	if(POT_LJ_SM_PRINT_FUNC>0) std::cout<<"ptnl::PotLJSm::compute(const Structure&,const NeighborList&):\n";
	double energy_=0;
	for(int i=0; i<struc.nAtoms(); ++i){
		const int ti=struc.type(i);
		for(int j=0; j<nlist.size(i); ++j){
			...
			if(nlist.neigh(i,j).dr()<rc_){
				...
				const double du6=du3*du3;
				energy_+=4.0*e_(ti,tj)*du6*(du6-1.0)-phirc_(ti,tj)-(dr-rc_)*dphirc_(ti,tj);
				const Eigen::Vector3d fij=(24.0*e_(ti,tj)*du6*(2.0*du6-1.0)*dri+dphirc_(ti,tj))*dri*r;
...
double PotLJSm::compute(Structure& struc, const NeighborList& nlist, int i){
	if(POT_LJ_SM_PRINT_FUNC>0) std::cout<<"ptnl::PotLJSm::compute(const Structure&,const NeighborList&,int):\n";
	double energy_=0;
	const int ti=struc.type(i);
	for(int j=0; j<nlist.size(i); ++j){
		const int tj=struc.type(nlist.neigh(i,j).index());
...
double PotLJSm::energy(const Structure& struc, const verlet::List& vlist){
	if(POT_LJ_SM_PRINT_FUNC>0) std::cout<<"ptnl::PotLJSm::energy(const Structure&,const verlet::List&):\n";
	Eigen::Vector3d drv;
	double energy_=0;
	for(int i=0; i<struc.nAtoms(); ++i){
		const int ti=struc.type(i);
...
double PotLJSm::energy(const Structure& struc, const verlet::List& vlist, int i){
	if(POT_LJ_SM_PRINT_FUNC>0) std::cout<<"ptnl::PotLJSm::energy(const Structure&,const verlet::List&,int):\n";
	Eigen::Vector3d drv;
	double energy_=0;
	const int ti=struc.type(i);
	for(int j=0; j<vlist.size(i); ++j){
...
double PotLJSm::compute(Structure& struc, const verlet::List& vlist){
	if(POT_LJ_SM_PRINT_FUNC>0) std::cout<<"ptnl::PotLJSm::compute(const Structure&,const verlet::List&):\n";
	Eigen::Vector3d drv;
	double energy_=0;
	for(int i=0; i<struc.nAtoms(); ++i){
		const int ti=struc.type(i);
		for(int j=0; j<vlist.size(i); ++j){
			...
			if(dr2<rc2_){
				...
				const double du6=du3*du3;
				energy_+=4.0*e_(ti,tj)*du6*(du6-1.0)-phirc_(ti,tj)-(dr-rc_)*dphirc_(ti,tj);
				const Eigen::Vector3d fij=(24.0*e_(ti,tj)*du6*(2.0*du6-1.0)*dri+dphirc_(ti,tj))*dri*drv;
				struc.force(i).noalias()+=fij;
			}
		}
	}
...
double PotLJSm::compute(Structure& struc, const verlet::List& vlist, int i){
	if(POT_LJ_SM_PRINT_FUNC>0) std::cout<<"ptnl::PotLJSm::compute(const Structure&,const verlet::List&,int):\n";
	Eigen::Vector3d drv;
	double energy_=0;
	const int ti=struc.type(i);
	for(int j=0; j<vlist.size(i); ++j){
...
namespace serialize{
	...
	template <> int nbytes(const ptnl::PotLJSm& obj){
		if(POT_LJ_SM_PRINT_FUNC>0) std::cout<<"nbytes(const ptnl::PotLJSm&):\n";
		int size=0;
	...
	template <> int pack(const ptnl::PotLJSm& obj, char* arr){
		if(POT_LJ_SM_PRINT_FUNC>0) std::cout<<"pack(const ptnl::PotLJSm&,char*):\n";
		int pos=0;
	...
	template <> int unpack(ptnl::PotLJSm& obj, const char* arr){
		if(POT_LJ_SM_PRINT_FUNC>0) std::cout<<"unpack(ptnl::PotLJSm&,const char*):\n";
		int pos=0,nt=0;
		...
		if(name!=ptnl::Pot::Name::LJ_CUT) throw std::invalid_argument("serialize::unpack(PotLJSm&,const char*): Invalid name.");
		obj.resize(nt);
		if(nt>0){
