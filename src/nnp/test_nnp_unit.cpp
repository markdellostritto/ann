...
void test_cutoff_cos(){
	...
	Basis basis(rc,Cutoff::Name::COS,1);
	//compute error
...
void test_unit_nnh(){
	...
	init.seed()=-1;
	init.neuron()=NN::Neuron::ARCTAN;
	nnh.nn().resize(init,nh);
	//initialize basis
	std::cout<<"initializing basis\n";
	const int nR=6;
	BasisR basisR(rc,Cutoff::Name::COS,nR,BasisR::Name::TANH);
	const int nA=4;
	BasisA basisA(rc,Cutoff::Name::COS,nA,BasisA::Name::GAUSS);
	for(int i=0; i<nR; ++i){
		basisR.rs(i)=1.06;
		basisR.eta(i)=i;
	}
