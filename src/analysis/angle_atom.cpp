...
int main(int argc, char* argv[]){
	...
	try{
				...
				for(int j=0; j<pair.size(atomi); ++j){
					...
					if(groupV.contains(atomj)){
						...
						if(1.0e-8<drij2 && drij2<blij2){
							for(int k=j+1; k<pair.size(atomi); ++k){
								const int atomk=pair.neigh(atomi,k);
								const double rk=radiit[sim.frame(t).type(atomk)];
								if(groupV.contains(atomk)){
									const Eigen::Vector3d rik=sim.frame(t).diff(sim.frame(t).posn(atomi),sim.frame(t).posn(atomk),dr2);
									const double drik2=rik.squaredNorm();
									const double blik2=(ri+rk)*(ri+rk);
									if(1.0e-8<drik2 && drik2<blik2){
										const double angle=std::acos(rij.dot(rik)/(rij.norm()*rik.norm()));
										angles[i].push(angle);
									}
								}
							}
		...
		//==== write the angles ====
		std::cout<<"writing the angles\n";
		FILE* writer=fopen("angle_atom.dat","w");
		if(writer!=NULL){
			fprintf(writer,"#name avg min max dev\n");
			for(int i=0; i<groupC.size(); ++i){
				fprintf(
					writer,"%s %12.8f %12.8f %12.8f %12.8f\n",
					sim.frame(0).name(groupC.atom(i)).c_str(),angles[i].avg(),angles[i].min(),angles[i].max(),angles[i].dev()
				);
			}
		}
