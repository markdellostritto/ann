...
int main(int argc, char* argv[]){
	...
	//batch
		...
		double pbatch=1;
	//thread dist
		parallel::Dist dist_batch;//subset - number of structures  - global - batch
		parallel::Dist dist_train;//dist - data - training
		parallel::Dist dist_val;  //dist - data - validation
		parallel::Dist dist_test; //dist - data - testing
	//neural network
		double vfrac=0.25;
		std::vector<int> nh;
